...
{
	PUCHAR OutputBuffer;
	UCHAR InputBuffer[LENGTH_INPUT_BUFFER];
	HANDLE hDevice;
	BOOL bRc;
	ULONG bytesReturned;
	DWORD errNum = 0;
...
Type \"help\" for getting help usage\r\n");
	...
	while (command != COMMAND_EXIT) {
		...
		UINT lengthOutputBuffer = command == COMMAND_READ ?
			(InputBuffer[1] - InputBuffer[0] + 1) :
			1;

		OutputBuffer = (PUCHAR)malloc(sizeof(UCHAR) * lengthOutputBuffer);

		if (OutputBuffer == NULL)
		{
			printf("Error when try allocate memory!\r\n");
			break;
		}

		bRc = FALSE;
		memset(OutputBuffer, 0, sizeof(UCHAR) * lengthOutputBuffer);
		...
		{
			ULONG key = 0;
			UCHAR lborder = InputBuffer[0], rborder = InputBuffer[1];
